...
### Core Technologies
- **Backend:** Erlang/OTP on FreeBSD (chosen for massive concurrency and fault tolerance)
- **Database:** Mnesia (distributed in-memory), Cassandra (persistent storage)
- **Protocol:** FunXMPP (binary optimized XMPP)
- **Encryption:** Signal Protocol for end-to-end encryption
- **Media:** H.264 video codec, Opus audio codec
- **Infrastructure:** Hosted on Facebook/Meta data centers globally

### Key Design Principles
1. **Simplicity Over Complexity:** Each component is focused and independent
2. **Asynchronous Everything:** Non-blocking operations throughout the stack
3. **Fault Isolation:** Failures contained to small islands, not cascading
4. **Memory-First:** 98% of messages served from RAM before touching disk
5. **Minimal Engineering Team:** Historically managed by 50-100 engineers total

### Erlang's Role
- Lightweight processes (not OS threads) → millions per server
- Supervision trees → automatic process recovery on failure
- Hot code swapping → deploy updates without downtime
- Built-in distribution → clusters span continents seamlessly

### Message Flow
1. **Send:** User → Local Erlang process → Serialization → Encryption
2. **Route:** Local server → Cluster routing → Recipient's server (via Mnesia lookup)
3. **Deliver:** Recipient's server → Persistent connection → Decryption → Display
4. **Total Latency:** Typically 100-500ms end-to-end globally